...
function decode_author($whole)
{
    ...
    return [join(", ", $names), $born, $died, $role];
}
    ...
    if ($filename = capture("filename", $e)) {
        ...
        */
        ...
        if (strpos($directory, "/$etext_number-")) {
            if (in_array($fk_filetypes, ["jpg", "png", "gif", "svg", "css", "xsl"])) {
                $diskstatus = 1;
            }
    ...
    if (!empty($etext_number)) {
        ...
        if ($updatemode == 0) {
            ...
            foreach ($lines as $line) {
                ...
                [$dummy, $key, $value] = $matches; // preg_split ("/:\s+/", $line); //FIXME
                if (empty($value)) {
                    continue;
                }

                if (isset($roles[$key])) {
                    [$author, $born, $died, $role] = decode_author($value);
