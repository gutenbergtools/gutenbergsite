...
In production, `_includes/latest_covers.html` and `_includes/book_count.html`
are generated by a web query on the prod system. For testing you can
`cp "_includes/latest_covers_test.html" "_includes/latest_covers.html"` and
`cp "_includes/book_count_test.html" "_includes/book_count.html"`

`latest_covers.html` and `book_count.html` are not tracked by git. There
are two ways to run a development environment. In either situation, as you
edit files, the server should notice and automatically build the changes into
