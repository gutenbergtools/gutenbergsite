...
In production, the following files are generated dynamically and not tracked by git:
* `_includes/latest_covers.html`
* `_includes/popular_covers.html`
* `_includes/ebook_count`

For testing you can copy over test files with:
```bash
cp _includes/latest_covers_test.html _includes/latest_covers.html
cp _includes/popular_covers_test.html _includes/popular_covers.html
cp _includes/ebook_count_test _includes/ebook_count
```
...
Once Ruby, Jekyll, and Git are setup install the dependencies with
```bash
bundle install
```
...
```bash
bundle exec jekyll build -d /path/to/destination
```

Or build and then serve it with the built-in web browser
```bash
bundle exec jekyll serve -d /path/to/destination
```

Then open your web browser to http://localhost:4000

As you edit files, the server will notice and automatically build the changes
into the site without a restart.

Without `-d` it will try to write it to the destination specified in `_config.yml`
which is unlikely to exist on your system.
